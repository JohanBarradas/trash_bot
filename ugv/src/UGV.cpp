...
public:
	...
	void read() {
		...
		vel[0] = (ret+8.0)*vel_in_value; //add 8 to match the current RPM we are getting
		//std::cerr << "wheel 0= "<< ret << "r/s: " << vel[0] << std::endl;
		vel[1] = device.GetValue(_ABSPEED,2,ret);
		vel[1] = ret*vel_in_value;
		//std::cerr << "wheel 1= "<< ret << "r/s: " << vel[1] << std::endl;
